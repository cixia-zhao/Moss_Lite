...
def run():
    ...
    print("[Link] 1. 开始校验并安装 Python 后端依赖包...")
    requirements_path = os.path.join(backend_dir, "requirements.txt")
    subprocess.run([sys.executable, "-m", "pip", "install", "-r", requirements_path], check=True)

    print("[Link] 2. 开始校验并安装 React 前端依赖包...")
    if not os.path.exists(os.path.join(frontend_dir, "node_modules")):
        print("[Link] node_modules 目录不存在，正执行安装...")
        subprocess.run("npm install", shell=True, cwd=frontend_dir, check=True)
    else:
        print("[Link] 检测到 node_modules 目录已存在，跳过 npm install 步骤以极速启动。")

    print("[Link] 3. 开始编译 React 前端静态资源...")
    subprocess.run("npm run build", shell=True, cwd=frontend_dir, check=True)

    print("[Link] 4. 开始部署静态资源到 FastAPI 容器中...")
